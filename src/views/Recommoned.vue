<template>
	<div>
		<div v-for="(item, index) in recommenedList" :key="index" class="card p-3 fill-white tiny-round  border-bottom">
...
export default {
	...
	created() {
		// this.axios.get('http://localhost:8888/recommend.json').then(res => {
		// 	console.log(res);
		// 	 this.recommenedList = res.data.data;
		// });
		this.axios.get('/api/recommend').then(res => {
			this.recommenedList = res.data.data.data;
			// console.log(this.recommenedList);
		});
	}
};
</script>
